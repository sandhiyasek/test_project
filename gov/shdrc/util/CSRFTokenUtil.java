...
public final class CSRFTokenUtil
{
   ...
   */
         ...
         */
         ...
         throws ServletException, NoSuchAlgorithmException,SHDRCException {
                    //throw exception if session is null
                    if (request.getSession(false)== null) {
                        throw new ServletException(NO_SESSION_ERROR);
                    }
                    String session = request.getParameter(SESSION_ATTR_KEY);
                    boolean isValid=getToken(request.getSession(false)).equals(
                                request.getParameter(SESSION_ATTR_KEY));
                    if(!isValid){
                    	throw new SHDRCException("Invalid form submission.Please contact System Administrator");
                    }
                    return isValid;
         }
