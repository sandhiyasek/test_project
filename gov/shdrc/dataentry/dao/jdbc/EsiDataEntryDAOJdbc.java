...
@Service
public class EsiDataEntryDAOJdbc implements IEsiDataEntryDAO {
private static Logger log=Logger.getLogger(EsiDataEntryDAOJdbc.class);
/*private static final Logger logger = Logger.getLogger(EsiDataEntryDAOJdbc.class);*/
@Autowired
...
ICommonDAO commonDAO;
	...
	public List<CommonStringList> getInstitutionList(Integer districtId,String userName){
		...
		try {
			...
			preparedStatement = connection.prepareStatement(ShdrcQueryList.ESI_INSTITUTION);
	...
	public boolean isInstitutionFUllAccess(Integer districtId,String userName){
		...
		try {
			...
			connection=jdbcTemplate.getDataSource().getConnection();
			preparedStatement = connection.prepareStatement(ShdrcQueryList.ESI_INSTITUTION_COUNT);
			preparedStatement.setInt(1,districtId);
			resultSet = preparedStatement.executeQuery();
	        if (resultSet.next()) {
	        	totalNoOfInstitution = resultSet.getInt(1);
	        }
			preparedStatement1 = connection.prepareStatement(ShdrcQueryList.ACCESS_RESTRICTED_ESI_INSTITUTION_COUNT);
		...
		try {
			...
			if(Util.isNotNull(districtId)){
				if(ShdrcConstants.DAILY.equalsIgnoreCase(frequency)){
					...
					preparedStatement = connection.prepareStatement(ShdrcQueryList.ESI_DAILY_RECORDS);
					preparedStatement.setDate(5, sqlDate);
				}else if(ShdrcConstants.WEEKLY.equalsIgnoreCase(frequency)){
					preparedStatement = connection.prepareStatement(ShdrcQueryList.ESI_WEEKLY_RECORDS);
					...
					preparedStatement = connection.prepareStatement(ShdrcQueryList.ESI_MONTHLY_RECORDS);
					...
					preparedStatement = connection.prepareStatement(ShdrcQueryList.ESI_QUARTERLY_RECORDS);
			...
			log.error(this.getClass().getName() + "- getEsiDirectorateRecords "+e.getMessage());
		}finally{
				try {
	                if (resultSet != null) {
	                    resultSet.close();
	...
	public boolean insertEsiDirectorateRecords(Integer districtId,Integer institutionId,String classificationName,String frequency,String week,String quarter,
		...
		try {
			...
			preparedStatementInsert = connection.prepareStatement
				      (ShdrcQueryList.INSERT_ESI_DIRECTORATE);
			...
			for(int i=1;i<gridRecords.length()-1;i++){
				if(!gridRecords.isNull(i)){
					...
					if(!gridRow.getBoolean("issucategorylabel")){
						...
						preparedStatementInsert.setInt(1,ShdrcConstants.DirectorateId.ESI);
			...
			log.error(this.getClass().getName() + "- insertEsiDirectorateRecords "+e.getMessage());
			try {
				connection.rollback();
			} catch (SQLException e1) {
				log.error(this.getClass().getName() + "- insertEsiDirectorateRecords "+e1.getMessage());
			}
	...
	public boolean updateEsiDirectorateRecords(JSONArray jsonArray){
		...
		try {
			...
			connection=jdbcTemplate.getDataSource().getConnection();
			connection.setAutoCommit(false);
			preparedStatementUpdate=connection.prepareStatement(ShdrcQueryList.UPDATE_ESI_DIRECTORATE);
			...
			log.error(this.getClass().getName() + "- updateEsiDirectorateRecords "+e.getMessage());
			try {
				connection.rollback();
			} catch (SQLException e1) {
				log.error(this.getClass().getName() + "- updateEsiDirectorateRecords "+e1.getMessage());
			}
		}catch(JSONException json){
			try {
				connection.rollback();
				log.error(this.getClass().getName() + "- updateEsiDirectorateRecords "+json.getMessage());
			} catch (SQLException e1) {
				log.error(this.getClass().getName() + "- updateEsiDirectorateRecords "+e1.getMessage());
			}
	...
	public JSONArray getIndicatorList(int directorateId,String classificationName,String frequency,String isDemographic,String generalNameLevel){
		...
		try {
			log.debug(this.getClass().getName() + "- Entering ");
			connection=jdbcTemplate.getDataSource().getConnection();
			preparedStatement = connection.prepareStatement(ShdrcQueryList.ESI_INDICATOR_LIST);
