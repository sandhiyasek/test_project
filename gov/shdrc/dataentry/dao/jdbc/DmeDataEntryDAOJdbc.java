...
@Service
public class DmeDataEntryDAOJdbc implements IDmeDataEntryDAO {
private static Logger log=Logger.getLogger(DmeDataEntryDAOJdbc.class);
/*private static final Logger logger = Logger.getLogger(DmeDataEntryDAOJdbc.class);*/
@Autowired
...
public List<CommonStringList> getInstitutionList(Integer districtId,String userName){
	...
	try {
		...
		preparedStatement = connection.prepareStatement(ShdrcQueryList.DME_INSTITUTION);
...
public boolean isInstitutionFUllAccess(Integer districtId,String userName){
	...
	try {
		...
		connection=jdbcTemplate.getDataSource().getConnection();
		preparedStatement = connection.prepareStatement(ShdrcQueryList.DME_INSTITUTION_COUNT);
		preparedStatement.setInt(1,districtId);
		resultSet = preparedStatement.executeQuery();
        if (resultSet.next()) {
        	totalNoOfInstitution = resultSet.getInt(1);
        }
		preparedStatement1 = connection.prepareStatement(ShdrcQueryList.ACCESS_RESTRICTED_DME_INSTITUTION_COUNT);
        ...
        flag=(totalNoOfInstitution!=1&&totalNoOfInstitution==totalNoOfUserAccessInstitution); 
		log.debug(this.getClass().getName() + "- Exit ");
	} catch (SQLException e) {
	...
	try {
		...
		if(Util.isNotNull(districtId)){
			if(ShdrcConstants.DAILY.equalsIgnoreCase(frequency)){
				...
				preparedStatement = connection.prepareStatement(ShdrcQueryList.DME_DAILY_RECORDS);
				preparedStatement.setDate(5, sqlDate);
			}else if(ShdrcConstants.WEEKLY.equalsIgnoreCase(frequency)){
				preparedStatement = connection.prepareStatement(ShdrcQueryList.DME_WEEKLY_RECORDS);
				...
				preparedStatement = connection.prepareStatement(ShdrcQueryList.DME_MONTHLY_RECORDS);
				...
				preparedStatement = connection.prepareStatement(ShdrcQueryList.DME_QUARTERLY_RECORDS);
		...
		log.error(this.getClass().getName() + "- getDmeDirectorateRecords "+e.getMessage());
	}
	finally{
			try {
                if (resultSet != null) {
...
String date,String month,Integer year,JSONArray gridRecords){ 
	...
	try {
		...
		preparedStatementInsert = connection.prepareStatement
			      (ShdrcQueryList.INSERT_DME_DIRECTORATE);
		...
		for(int i=1;i<gridRecords.length()-1;i++){
			if(!gridRecords.isNull(i)){
				...
				if(!gridRow.getBoolean("issucategorylabel")){
					...
					preparedStatementInsert.setInt(1,ShdrcConstants.DirectorateId.DME);
		...
		log.error(this.getClass().getName() + "- insertDmeDirectorateRecords "+e.getMessage());
		try {
			connection.rollback();
		} catch (SQLException e1) {
			log.error(this.getClass().getName() + "- insertDmeDirectorateRecords "+e1.getMessage());
		}
...
public boolean updateDmeDirectorateRecords(JSONArray jsonArray){
	...
	try {
		...
		connection=jdbcTemplate.getDataSource().getConnection();
		connection.setAutoCommit(false);
		preparedStatementUpdate=connection.prepareStatement(ShdrcQueryList.UPDATE_DME_DIRECTORATE);
		...
		log.error(this.getClass().getName() + "- updateDmeDirectorateRecords "+e.getMessage());
		try {
			connection.rollback();
		} catch (SQLException e1) {
			log.error(this.getClass().getName() + "- updateDmeDirectorateRecords "+e1.getMessage());
		}
	}catch(JSONException json){
		try {
			connection.rollback();
			log.error(this.getClass().getName() + "- updateDmeDirectorateRecords "+json.getMessage());
		} catch (SQLException e1) {
			log.error(this.getClass().getName() + "- updateDmeDirectorateRecords "+e1.getMessage());
		}		
	}finally{
...
public JSONArray getIndicatorList(int directorateId,String classificationName,String frequency,String isDemographic,
	...
	try {
		log.debug(this.getClass().getName() + "- Entering ");
		connection=jdbcTemplate.getDataSource().getConnection();
		preparedStatement = connection.prepareStatement(ShdrcQueryList.DME_INDICATOR_LIST);
