...
@Service
public class RntcpDataEntryDAOJdbc implements IRntcpDataEntryDAO {
private static Logger log=Logger.getLogger(RntcpDataEntryDAOJdbc.class);
/*private static final Logger logger = Logger.getLogger(RntcpDataEntryDAOJdbc.class);*/
@Autowired
...
ICommonDAO commonDAO;
	...
	public List<CommonStringList> getInstitutionList(Integer districtId,String userName){
		...
		try {
			...
			preparedStatement = connection.prepareStatement(ShdrcQueryList.RNTCP_INSTITUTION);
	...
	public boolean isInstitutionFUllAccess(Integer districtId,String userName){
		...
		try {
			...
			connection=jdbcTemplate.getDataSource().getConnection();
			preparedStatement = connection.prepareStatement(ShdrcQueryList.RNTCP_INSTITUTION_COUNT);
			preparedStatement.setInt(1,districtId);
			resultSet = preparedStatement.executeQuery();
	        if (resultSet.next()) {
	        	totalNoOfInstitution = resultSet.getInt(1);
	        }
			preparedStatement1 = connection.prepareStatement(ShdrcQueryList.ACCESS_RESTRICTED_RNTCP_INSTITUTION_COUNT);
		...
		try {	
			...
			if(Util.isNotNull(districtId)){
				if(ShdrcConstants.DAILY.equalsIgnoreCase(frequency)){
					...
					preparedStatement = connection.prepareStatement(ShdrcQueryList.RNTCP_DAILY_RECORDS);
					preparedStatement.setDate(5, sqlDate);
				}else if(ShdrcConstants.WEEKLY.equalsIgnoreCase(frequency)){
					preparedStatement = connection.prepareStatement(ShdrcQueryList.RNTCP_WEEKLY_RECORDS);
					...
					preparedStatement = connection.prepareStatement(ShdrcQueryList.RNTCP_MONTHLY_RECORDS);
					...
					preparedStatement = connection.prepareStatement(ShdrcQueryList.RNTCP_QUARTERLY_RECORDS);
			...
			log.error(this.getClass().getName() + "- getRntcpDirectorateRecords "+e.getMessage());
		}finally{
				try {
	                if (resultSet != null) {
	                    resultSet.close();
	...
	public boolean insertRntcpDirectorateRecords(Integer districtId,Integer institutionId,String classificationName,String dataEntryLevel,
		...
		try {
			...
			preparedStatementInsert = connection.prepareStatement
				      (ShdrcQueryList.INSERT_RNTCP_DIRECTORATE);
			...
			for(int i=1;i<gridRecords.length()-1;i++){
				if(!gridRecords.isNull(i)){
					...
					if(!gridRow.getBoolean("issucategorylabel")){
						...
						preparedStatementInsert.setInt(1,ShdrcConstants.DirectorateId.RNTCP);
			...
			log.error(this.getClass().getName() + "- insertRntcpDirectorateRecords "+e.getMessage());
			try {
				connection.rollback();
			} catch (SQLException e1) {
				log.error(this.getClass().getName() + "- insertRntcpDirectorateRecords "+e1.getMessage());
			}
	...
	public boolean updateRntcpDirectorateRecords(JSONArray jsonArray){
		...
		try {
			...
			connection=jdbcTemplate.getDataSource().getConnection();
			connection.setAutoCommit(false);
			preparedStatementUpdate=connection.prepareStatement(ShdrcQueryList.UPDATE_RNTCP_DIRECTORATE);
			...
			log.error(this.getClass().getName() + "- updateRntcpDirectorateRecords "+e.getMessage());
			try {
				connection.rollback();
			} catch (SQLException e1) {
				log.error(this.getClass().getName() + "- updateRntcpDirectorateRecords "+e1.getMessage());
			}
		}catch(JSONException json){
			try {
				connection.rollback();
				log.error(this.getClass().getName() + "- updateRntcpDirectorateRecords "+json.getMessage());
			} catch (SQLException e1) {
				log.error(this.getClass().getName() + "- updateRntcpDirectorateRecords "+e1.getMessage());
			}			
		}finally{
	...
	public JSONArray getIndicatorList(int directorateId,String classificationName,String frequency,String isDemographic,String dataEntryLevel){
		...
		try {
			log.debug(this.getClass().getName() + "- Entering ");
			connection=jdbcTemplate.getDataSource().getConnection();
			preparedStatement = connection.prepareStatement(ShdrcQueryList.RNTCP_INDICATOR_LIST);
	...
	public List<CommonStringList> getDistricts(String userName){
		...
		try {
			...
			connection=jdbcTemplate.getDataSource().getConnection();
			preparedStatement = connection.prepareStatement(ShdrcQueryList.RNTCP_DISTRICT_NAME);
	...
	public boolean isDistrictsFUllAccess(String userName){
		...
		try {
			...
			connection=jdbcTemplate.getDataSource().getConnection();
			preparedStatement = connection.prepareStatement(ShdrcQueryList.RNTCP_DISTRICT_NAME_COUNT);
			resultSet = preparedStatement.executeQuery();
	        if (resultSet.next()) {
	        	totalNoOfDistricts = resultSet.getInt(1);
	        }
			preparedStatement1 = connection.prepareStatement(ShdrcQueryList.RESTRICTED_RNTCP_DISTRICT_NAME_COUNT);
			...
			log.error(this.getClass().getName() + "- isDistrictsFUllAccess "+e.getMessage());
		}
		catch(Exception json){
