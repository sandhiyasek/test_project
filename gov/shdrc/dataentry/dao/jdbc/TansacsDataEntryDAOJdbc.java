...
@Service
public class TansacsDataEntryDAOJdbc implements ITansacsDataEntryDAO {
private static Logger log=Logger.getLogger(TansacsDataEntryDAOJdbc.class);
/*private static final Logger logger = Logger.getLogger(TansacsDataEntryDAOJdbc.class);*/
@Autowired
...
ICommonDAO commonDAO;
	...
	public List<CommonStringList> getInstitutionList(Integer districtId,String institutionType,String userName){
		...
		try {
			...
			preparedStatement = connection.prepareStatement(ShdrcQueryList.AIDS_INSTITUTION);
	...
	public boolean isInstitutionFUllAccess(Integer districtId,String userName){
		...
		try {
			...
			connection=jdbcTemplate.getDataSource().getConnection();
			preparedStatement = connection.prepareStatement(ShdrcQueryList.AIDS_INSTITUTION_COUNT);
			preparedStatement.setInt(1,districtId);
			resultSet = preparedStatement.executeQuery();
	        if (resultSet.next()) {
	        	totalNoOfInstitution = resultSet.getInt(1);
	        }
			preparedStatement1 = connection.prepareStatement(ShdrcQueryList.ACCESS_RESTRICTED_AIDS_INSTITUTION_COUNT);
	        ...
	        if (resultSet.next()) {
	        	totalNoOfUserAccessInstitution = resultSet.getInt(1);
	        }
	...
	public List<CommonStringList> getInstitutionTypeList(Integer districtId,String userName){
		...
		try {
			...
			connection=jdbcTemplate.getDataSource().getConnection();
			preparedStatement = connection.prepareStatement(ShdrcQueryList.AIDS_INSTITUTION_TYPE);
			preparedStatement.setInt(1,districtId);
		...
		try {
			...
			if(Util.isNotNull(districtId)){
				if(ShdrcConstants.DAILY.equalsIgnoreCase(frequency)){
					...
					preparedStatement = connection.prepareStatement(ShdrcQueryList.AIDS_DAILY_RECORDS);
					preparedStatement.setDate(7, sqlDate);
				}else if(ShdrcConstants.WEEKLY.equalsIgnoreCase(frequency)){
					preparedStatement = connection.prepareStatement(ShdrcQueryList.AIDS_WEEKLY_RECORDS);
					...
					preparedStatement = connection.prepareStatement(ShdrcQueryList.AIDS_MONTHLY_RECORDS);
					...
					preparedStatement = connection.prepareStatement(ShdrcQueryList.AIDS_QUARTERLY_RECORDS);
				...
				preparedStatement.setString(6, isDemographic);
				
				resultSet = preparedStatement.executeQuery();
				jsonArray = commonDAO.getGridRecordsFromRS(resultSet);
			}log.debug(this.getClass().getName() + "- Exit ");
		} catch (SQLException e) {
			log.error(this.getClass().getName() + "- getAidsDirectorateRecords "+e.getMessage());
		}finally{
				try {
	                if (resultSet != null) {
	                    resultSet.close();
	...
	public boolean insertAidsDirectorateRecords(Integer districtId,String institutionType,Integer institutionId,String classificationName,
		...
		try {
			...
			preparedStatementInsert = connection.prepareStatement
				      (ShdrcQueryList.INSERT_AIDS_DIRECTORATE);
			...
			for(int i=1;i<gridRecords.length()-1;i++){
				if(!gridRecords.isNull(i)){
					...
					if(!gridRow.getBoolean("issucategorylabel")){
						 ...
						 preparedStatementInsert.setInt(1,ShdrcConstants.DirectorateId.TANSACS);
						 preparedStatementInsert.setString(2,dataEntryLevel);
							...
							preparedStatementInsert.setString(7,"N");
							preparedStatementInsert.setInt(8,generalId);
							preparedStatementInsert.setString(9,generalType);
							preparedStatementInsert.setString(10,generalName);
							preparedStatementInsert.setString(11,subCategory);
							preparedStatementInsert.setInt(12,indicatorId);
							preparedStatementInsert.setString(13,indicatorName);
							preparedStatementInsert.setString(14,isCharacter);
							if(indicatorValue==null)
								preparedStatementInsert.setNull(15,java.sql.Types.DOUBLE);
							else
								preparedStatementInsert.setDouble(15,indicatorValue);
							preparedStatementInsert.setString(16,indicatorValue1);
							preparedStatementInsert.setString(17,frequency);
							preparedStatementInsert.setDate(18,sqlDate);
							preparedStatementInsert.setString(19,Util.isNullOrBlank(week)?null:week);
							preparedStatementInsert.setString(20,Util.isNullOrBlank(quarter)?null:quarter);
							month = Util.isNullOrBlank(month)?null:month;
							preparedStatementInsert.setString(21,month);
							if(year==null)
								preparedStatementInsert.setNull(22,java.sql.Types.INTEGER);
							else
								preparedStatementInsert.setInt(22,year);
							java.sql.Timestamp addOnDate = new java.sql.Timestamp(new Date().getTime());
							preparedStatementInsert.setTimestamp(23,addOnDate);
							preparedStatementInsert.setString(24, UserInfo.getLoggedInUserName());
	    	...
	    	log.error(this.getClass().getName() + "- insertAidsDirectorateRecords "+e.getMessage());
			try {
				connection.rollback();
			} catch (SQLException e1) {
				log.error(this.getClass().getName() + "- insertAidsDirectorateRecords "+e1.getMessage());
			}
	...
	public boolean updateAidsDirectorateRecords(JSONArray jsonArray){
		...
		try {
			...
			connection=jdbcTemplate.getDataSource().getConnection();
			connection.setAutoCommit(false);
			preparedStatementUpdate=connection.prepareStatement(ShdrcQueryList.UPDATE_AIDS_DIRECTORATE);
	    	...
	    	log.error(this.getClass().getName() + "- updateAidsDirectorateRecords "+e.getMessage());
			try {
				connection.rollback();
			} catch (SQLException e1) {
				log.error(this.getClass().getName() + "- updateAidsDirectorateRecords "+e1.getMessage());
			}
		}catch(JSONException json){
			try {
				connection.rollback();
				log.error(this.getClass().getName() + "- updateAidsDirectorateRecords "+json.getMessage());
			} catch (SQLException e1) {
				log.error(this.getClass().getName() + "- updateAidsDirectorateRecords "+e1.getMessage());
			}
	...
	public JSONArray getIndicatorList(int directorateId,String classificationName,String frequency,String isDemographic,String dataEntryLevel){
		...
		try {
			log.debug(this.getClass().getName() + "- Entering ");
			connection=jdbcTemplate.getDataSource().getConnection();
			preparedStatement = connection.prepareStatement(ShdrcQueryList.AIDS_INDICATOR_LIST);
