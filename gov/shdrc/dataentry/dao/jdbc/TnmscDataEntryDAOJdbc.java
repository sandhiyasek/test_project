...
@Service
public class TnmscDataEntryDAOJdbc implements ITnmscDataEntryDAO {
private static Logger log=Logger.getLogger(TnmscDataEntryDAOJdbc.class);
/*private static final Logger logger = Logger.getLogger(TnmscDataEntryDAOJdbc.class);*/
@Autowired
...
public List<CommonStringList> getInstitutionList(){
	...
	try {
		...
		preparedStatement = connection.prepareStatement(ShdrcQueryList.SELECT_TNMSC_INSTITUTION);
...
public JSONArray getTnmscDirectorateRecords(Integer districtId,Integer institutionId,String classificationName,String frequency,String week,String quarter,
	...
	PreparedStatement preparedStatement =null;
	ResultSet resultSet =null;
	districtId = ShdrcConstants.NAID;
	try {
		log.debug(this.getClass().getName() + "- Entering ");
		connection=jdbcTemplate.getDataSource().getConnection();
		if(Util.isNotNull(districtId)){
			if(ShdrcConstants.DAILY.equalsIgnoreCase(frequency)){
				...
				preparedStatement = connection.prepareStatement(ShdrcQueryList.TNMSC_DAILY_RECORDS);
				preparedStatement.setDate(5, sqlDate);
			}else if(ShdrcConstants.WEEKLY.equalsIgnoreCase(frequency)){
				preparedStatement = connection.prepareStatement(ShdrcQueryList.TNMSC_WEEKLY_RECORDS);
				...
				preparedStatement = connection.prepareStatement(ShdrcQueryList.TNMSC_MONTHLY_RECORDS);
				...
				preparedStatement = connection.prepareStatement(ShdrcQueryList.TNMSC_QUARTERLY_RECORDS);
		...
		log.error(this.getClass().getName() + "- getTnmscDirectorateRecords "+e.getMessage());
	}finally{
			try {
                if (resultSet != null) {
                    resultSet.close();
	...
	try {
		...
		preparedStatementInsert = connection.prepareStatement
			      (ShdrcQueryList.INSERT_TNMSC_DIRECTORATE);
		...
		for(int i=1;i<gridRecords.length()-1;i++){
			if(!gridRecords.isNull(i)){
				...
				if(!gridRow.getBoolean("issucategorylabel")){
					...
					preparedStatementInsert.setInt(1,ShdrcConstants.DirectorateId.TNMSC);
    	...
    	log.error(this.getClass().getName() + "- insertTnmscDirectorateRecords "+e.getMessage());
		try {
			connection.rollback();
		} catch (SQLException e1) {
			log.error(this.getClass().getName() + "- insertTnmscDirectorateRecords "+e1.getMessage());
		}
...
public boolean updateTnmscDirectorateRecords(JSONArray jsonArray){
	...
	try {
		...
		connection=jdbcTemplate.getDataSource().getConnection();
		connection.setAutoCommit(false);
		preparedStatementUpdate=connection.prepareStatement(ShdrcQueryList.UPDATE_TNMSC_DIRECTORATE);
		String isCharacter=null;
		String indicatorValue1=null;
		Double indicatorValue=null;
    	...
    	log.error(this.getClass().getName() + "- updateTnmscDirectorateRecords "+e.getMessage());
		try {
			connection.rollback();
		} catch (SQLException e1) {
	    	log.error(this.getClass().getName() + "- updateTnmscDirectorateRecords "+e1.getMessage());
		}
	}catch(JSONException json){
		try {
			connection.rollback();
	    	log.error(this.getClass().getName() + "- updateTnmscDirectorateRecords "+json.getMessage());
		} catch (SQLException e1) {
	    	log.error(this.getClass().getName() + "- updateTnmscDirectorateRecords "+e1.getMessage());
		}
...
public JSONArray getIndicatorList(int directorateId,String classificationName,String frequency,String isDemographic){
	...
	try {
		log.debug(this.getClass().getName() + "- Entering ");
		connection=jdbcTemplate.getDataSource().getConnection();
		preparedStatement = connection.prepareStatement(ShdrcQueryList.TNMSC_INDICATOR_LIST);
