...
@Service
public class NrhmDataEntryDAOJdbc implements INrhmDataEntryDAO {
private static Logger log=Logger.getLogger(NrhmDataEntryDAOJdbc.class);
/*private static final Logger logger = Logger.getLogger(NrhmDataEntryDAOJdbc.class);*/
@Autowired
...
ICommonDAO commonDAO;
		...
		try {
			...
			if(Util.isNotNull(districtId)){
				if(ShdrcConstants.DAILY.equalsIgnoreCase(frequency)){
					...
					preparedStatement = connection.prepareStatement(ShdrcQueryList.NRHM_DAILY_RECORDS);
					preparedStatement.setDate(7, sqlDate);
				}else if(ShdrcConstants.WEEKLY.equalsIgnoreCase(frequency)){
					preparedStatement = connection.prepareStatement(ShdrcQueryList.NRHM_WEEKLY_RECORDS);
					...
					preparedStatement = connection.prepareStatement(ShdrcQueryList.NRHM_MONTHLY_RECORDS);
					...
					preparedStatement = connection.prepareStatement(ShdrcQueryList.NRHM_QUARTERLY_RECORDS);
			...
			log.error(this.getClass().getName() + "- getNrhmDirectorateRecords "+e.getMessage());
		}finally{
				try {
	                if (resultSet != null) {
	                    resultSet.close();
	...
	public boolean insertNrhmDirectorateRecords(Integer districtId,String institutionType,Integer institutionId,String classificationName,
		...
		try {
			...
			preparedStatementInsert = connection.prepareStatement
				      (ShdrcQueryList.INSERT_NRHM_DIRECTORATE);
			...
			for(int i=1;i<gridRecords.length()-1;i++){
				if(!gridRecords.isNull(i)){
					...
					if(!gridRow.getBoolean("issucategorylabel")){
						...
						preparedStatementInsert.setInt(1,ShdrcConstants.DirectorateId.NRHM);
			...
			log.error(this.getClass().getName() + "- insertNrhmDirectorateRecords "+e.getMessage());
			try {
				connection.rollback();
			} catch (SQLException e1) {
				log.error(this.getClass().getName() + "- insertNrhmDirectorateRecords "+e1.getMessage());
			}
	...
	public boolean updateNrhmDirectorateRecords(JSONArray jsonArray){
		...
		try {
			...
			connection=jdbcTemplate.getDataSource().getConnection();
			connection.setAutoCommit(false);
			preparedStatementUpdate=connection.prepareStatement(ShdrcQueryList.UPDATE_NRHM_DIRECTORATE);
			...
			log.error(this.getClass().getName() + "- updateNrhmDirectorateRecords "+e.getMessage());
			try {
				connection.rollback();
			} catch (SQLException e1) {
				log.error(this.getClass().getName() + "- updateNrhmDirectorateRecords "+e1.getMessage());
			}
		}catch(JSONException json){
			try {
				connection.rollback();
				log.error(this.getClass().getName() + "- updateNrhmDirectorateRecords "+json.getMessage());
			} catch (SQLException e1) {
				log.error(this.getClass().getName() + "- updateNrhmDirectorateRecords "+e1.getMessage());
			}			
		}finally{
	...
	public JSONArray getIndicatorList(int directorateId,String classificationName,String frequency,String isDemographic){
		...
		try {
			log.debug(this.getClass().getName() + "- Entering ");
			connection=jdbcTemplate.getDataSource().getConnection();
			preparedStatement = connection.prepareStatement(ShdrcQueryList.NRHM_INDICATOR_LIST);
