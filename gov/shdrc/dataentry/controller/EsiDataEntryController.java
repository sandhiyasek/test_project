...
@Controller
public class EsiDataEntryController {
	private static Logger log=Logger.getLogger(EsiDataEntryController.class);
	@Autowired
	IEsiDataEntryManager esiDataEntryManager;
	@Autowired
	ICommonManager commonManager;
	
	@RequestMapping(value="/esiDataEntry", method=RequestMethod.GET)
	public ModelAndView onLoad(HttpServletRequest request) throws SHDRCException, NoSuchAlgorithmException, ServletException{		
		...
		try { 
		    ...
		    UserInfo.hasValidDataEntryAccess(ShdrcConstants.Role.ESI);
		    modelView.setViewName("esiDataentry");			
	        ...
	        if(Util.isNotNull(districtList)){
	        	...
	        	if(ShdrcConstants.SELECTALLID==district.getId()){
	        		...
	        		institutionList= esiDataEntryManager.getInstitutionList(district.getId(),userName);
	        	}
	        ...
	        modelView.addObject("directorateId", ShdrcConstants.DirectorateId.ESI);
	        
	        List<CommonStringList> classificationList= commonManager.getClassificationList(ShdrcConstants.DirectorateId.ESI,ShdrcQueryList.ESI_CLASSIFICATION_LIST);
	...
	@RequestMapping(value="/esidistrictChange", method=RequestMethod.POST)
	public @ResponseBody void getInstitution(HttpServletRequest request,HttpServletResponse response) throws SHDRCException, IOException, JSONException{
		...
		try { 
			...
			if(Util.isNotNull(request.getParameter("districtId"))){
				...
				institutionList= esiDataEntryManager.getInstitutionList(districtId,userName);
			}else{
				institutionList = new ArrayList<CommonStringList>();
				CommonStringList commonStringList = new CommonStringList();
				commonStringList.setId(ShdrcConstants.SELECTALLID);
	...
	@RequestMapping(value="/esisearch", method=RequestMethod.POST)
	public @ResponseBody void search(HttpServletRequest request,HttpServletResponse response) throws SHDRCException, IOException, JSONException{
		try { 
			...
			JSONArray esiDirectorateRecords =null;
			
			classificationList=commonManager.getGeneralCategoryListByFrequency(ShdrcConstants.DirectorateId.ESI,frequency,dataEntryLevel,ShdrcQueryList.ESI_GENERAL_CATEGORY_LIST_BY_FREQUENCY);
			if(classificationList!=null){
				CommonStringList commonStringList = classificationList.get(0);
		        classificationName = commonStringList.getName();
		        esiDirectorateRecords = esiDataEntryManager.getEsiDirectorateRecords(districtId,institutionId,classificationName,frequency,week,quarter,searchDate, month, year);
		        if(esiDirectorateRecords==null){
		    		esiDirectorateRecords = esiDataEntryManager.getIndicatorList(ShdrcConstants.DirectorateId.ESI,classificationName,frequency,"N",null);
		    	}
			 } 			
			...
			obj.put("esiDirectorateRecords", esiDirectorateRecords);
	...
	@RequestMapping(value="/esitabChange", method=RequestMethod.POST)
	public @ResponseBody void onTabChange(HttpServletRequest request,HttpServletResponse response) throws SHDRCException, IOException, JSONException{
		try { 
			...
			obj.put("isFrequencySelectionValid", true);
			json=new Gson().toJson(classificationList);
			JSONArray esiDirectorateRecords = esiDataEntryManager.getEsiDirectorateRecords(districtId,institutionId,classificationName,frequency,week,quarter,
					 searchDate, month, year);
			if(esiDirectorateRecords==null && classificationName!=null){
				esiDirectorateRecords = esiDataEntryManager.getIndicatorList(ShdrcConstants.DirectorateId.ESI,classificationName,frequency,"N",null);
			}
			obj.put("esiDirectorateRecords", esiDirectorateRecords);
			obj.put("genearlCategoryList", json);
	...
	@RequestMapping(value="/esisave", method=RequestMethod.POST)
	public @ResponseBody void doSave(HttpServletRequest request,HttpServletResponse response) throws SHDRCException, IOException, ServletException, JSONException{
		...
		PrintWriter out = response.getWriter();
			...
			boolean successFlag =esiDataEntryManager.insertEsiDirectorateRecords(districtId,institutionId, classificationName,frequency,week,quarter,
	...
	@RequestMapping(value="/esiupdate", method=RequestMethod.POST)
	public @ResponseBody void doUpdate(HttpServletRequest request,HttpServletResponse response) throws SHDRCException, IOException, ServletException, JSONException{
		try{
			...
			boolean successFlag = esiDataEntryManager.updateEsiDirectorateRecords(jsonArray);
			if(successFlag)
