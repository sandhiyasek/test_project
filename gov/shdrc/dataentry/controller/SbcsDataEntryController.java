...
@Controller
public class SbcsDataEntryController {
	private static Logger log= Logger.getLogger(SbcsDataEntryController.class);
	@Autowired
	ISbcsDataEntryManager sbcsDataEntryManager;
	@Autowired
	ICommonManager commonManager;
	
	@RequestMapping(value="/sbcsDataEntry", method=RequestMethod.GET)
	public ModelAndView onLoad(HttpServletRequest request) throws SHDRCException, NoSuchAlgorithmException, ServletException{		
		...
		try { 
		    ...
		    UserInfo.hasValidDataEntryAccess(ShdrcConstants.Role.SBCS);
		    modelView.setViewName("sbcsDataentry");	
	        ...
	        if(Util.isNotNull(districtList)){
	        	...
	        	if(ShdrcConstants.SELECTALLID==district.getId()){
	        		...
	        		institutionList= sbcsDataEntryManager.getInstitutionList(district.getId(),userName);
	        	}
	        ...
	        List<CommonStringList> classificationList= commonManager.getClassificationList(ShdrcConstants.DirectorateId.SBCS,ShdrcQueryList.SBCS_CLASSIFICATION_LIST);
	        ...
	        modelView.addObject("directorateId", ShdrcConstants.DirectorateId.SBCS);
	...
	@RequestMapping(value="/sbcsdistrictChange", method=RequestMethod.POST)
	public @ResponseBody void getInstitution(HttpServletRequest request,HttpServletResponse response) throws SHDRCException, IOException, JSONException{
		...
		try { 
			...
			if(Util.isNotNull(request.getParameter("districtId"))){
				...
				institutionList= sbcsDataEntryManager.getInstitutionList(districtId,userName);
			}else{
				institutionList = new ArrayList<CommonStringList>();
				CommonStringList commonStringList = new CommonStringList();
				commonStringList.setId(ShdrcConstants.SELECTALLID);
	...
	@RequestMapping(value="/sbcssearch", method=RequestMethod.POST)
	public @ResponseBody void search(HttpServletRequest request,HttpServletResponse response) throws SHDRCException, IOException, JSONException{
		try { 
			...
			JSONArray sbcsDirectorateRecords =null;
		
				 classificationList=commonManager.getGeneralCategoryListByFrequency(ShdrcConstants.DirectorateId.SBCS,frequency,dataEntryLevel,ShdrcQueryList.SBCS_GENERAL_CATEGORY_LIST_BY_FREQUENCY);
				 if(classificationList!=null){
			        	CommonStringList commonStringList = classificationList.get(0);
			        	classificationName = commonStringList.getName();
			        	sbcsDirectorateRecords = sbcsDataEntryManager.getSbcsDirectorateRecords(districtId,institutionId,classificationName,frequency,week,quarter,
			   				 searchDate, month, year);
			        	
			        	if(sbcsDirectorateRecords==null){
			    			sbcsDirectorateRecords = sbcsDataEntryManager.getIndicatorList(ShdrcConstants.DirectorateId.SBCS,classificationName,frequency,"N",null);
			    		}
				 } 				
			String json=null;
			if(classificationList!=null){
			...
			obj.put("sbcsDirectorateRecords", sbcsDirectorateRecords);
	...
	@RequestMapping(value="/sbcstabChange", method=RequestMethod.POST)
	public @ResponseBody void onTabChange(HttpServletRequest request,HttpServletResponse response) throws SHDRCException, IOException, JSONException{
		try { 
			...
			obj.put("isFrequencySelectionValid", true);
			json=new Gson().toJson(classificationList);
			JSONArray sbcsDirectorateRecords = sbcsDataEntryManager.getSbcsDirectorateRecords(districtId,institutionId,classificationName,frequency,week,quarter,
					 searchDate, month, year);
			
			if(sbcsDirectorateRecords==null && classificationName!=null){
				sbcsDirectorateRecords = sbcsDataEntryManager.getIndicatorList(ShdrcConstants.DirectorateId.SBCS,classificationName,frequency,"N",null);
			}
			obj.put("sbcsDirectorateRecords", sbcsDirectorateRecords);
			obj.put("genearlCategoryList", json);
	...
	@RequestMapping(value="/sbcssave", method=RequestMethod.POST)
	public @ResponseBody void doSave(HttpServletRequest request,HttpServletResponse response) throws SHDRCException, IOException, ServletException, JSONException{
		...
		PrintWriter out = response.getWriter();
			...
			boolean successFlag =sbcsDataEntryManager.insertSbcsDirectorateRecords(districtId,institutionId, classificationName,frequency,week,quarter,
	...
	@RequestMapping(value="/sbcsupdate", method=RequestMethod.POST)
	public @ResponseBody void doUpdate(HttpServletRequest request,HttpServletResponse response) throws SHDRCException, IOException, ServletException, JSONException{
		try{
			...
			boolean successFlag = sbcsDataEntryManager.updateSbcsDirectorateRecords(jsonArray);
			if(successFlag)
