...
@Controller
public class PrivateSectorDataEntryController {
	private static Logger log =Logger.getLogger(PrivateSectorDataEntryController.class);
	@Autowired
	IPrivateSectorDataEntryManager privateSectorDataEntryManager;
	@Autowired
	ICommonManager commonManager;
	
	@RequestMapping(value="/privateSectorDataEntry", method=RequestMethod.GET)
	public ModelAndView onLoad(HttpServletRequest request) throws NoSuchAlgorithmException, ServletException, SHDRCException{		
		...
		try { 
		    ...
		    UserInfo.hasValidDataEntryAccess(ShdrcConstants.Role.PVTSEC);
		    modelView.setViewName("privateSectorDataentry");			
	        ...
	        List<CommonStringList> classificationList= commonManager.getClassificationList(ShdrcConstants.DirectorateId.PVTSEC,ShdrcQueryList.PVTSEC_CLASSIFICATION_LIST);
	        ...
	        modelView.addObject("currentHours", Util.getCurrentHours());
	     	...
	     	modelView.addObject("directorateId", ShdrcConstants.DirectorateId.PVTSEC);  
	...
	@RequestMapping(value="/privateSectorsearch", method=RequestMethod.POST)
	public @ResponseBody void search(HttpServletRequest request,HttpServletResponse response) throws IOException, JSONException, SHDRCException{
		try { 
			...
			JSONArray pvtSecDirectorateRecords =null;
			
			 classificationList=commonManager.getGeneralCategoryListByFrequency(ShdrcConstants.DirectorateId.PVTSEC,frequency,dataEntryLevel,ShdrcQueryList.PVTSEC_GENERAL_CATEGORY_LIST_BY_FREQUENCY);
			 if(classificationList!=null){
		        	CommonStringList commonStringList = classificationList.get(0);
		        	classificationName = commonStringList.getName();
		        	pvtSecDirectorateRecords = privateSectorDataEntryManager.getPvtSecDirectorateRecords(districtId,institutionId,classificationName,frequency,week,quarter,
		   				 searchDate, month, year);
		        	if(pvtSecDirectorateRecords==null){
		        		pvtSecDirectorateRecords = privateSectorDataEntryManager.getIndicatorList(ShdrcConstants.DirectorateId.PVTSEC,classificationName,frequency,"N",null);
		    		}
			 } 			
			...
			obj.put("pvtSecDirectorateRecords", pvtSecDirectorateRecords);
	...
	@RequestMapping(value="/privateSectortabChange", method=RequestMethod.POST)
	public @ResponseBody void onTabChange(HttpServletRequest request,HttpServletResponse response) throws IOException, JSONException, SHDRCException{
		try { 
			...
			obj.put("isFrequencySelectionValid", true);
			json=new Gson().toJson(classificationList);
			JSONArray pvtSecDirectorateRecords = privateSectorDataEntryManager.getPvtSecDirectorateRecords(districtId,institutionId,classificationName,frequency,week,quarter,
					 searchDate, month, year);
			if(pvtSecDirectorateRecords==null && classificationName!=null){
				pvtSecDirectorateRecords = privateSectorDataEntryManager.getIndicatorList(ShdrcConstants.DirectorateId.PVTSEC,classificationName,frequency,"N",null);
			}
			obj.put("pvtSecDirectorateRecords", pvtSecDirectorateRecords);
			obj.put("genearlCategoryList", json);
	...
	@RequestMapping(value="/privateSectorsave", method=RequestMethod.POST)
	public @ResponseBody void doSave(HttpServletRequest request,HttpServletResponse response) throws SHDRCException, IOException, ServletException, JSONException{
		...
		try{
			...
			boolean successFlag =privateSectorDataEntryManager.insertPvtSecDirectorateRecords(districtId,institutionId, classificationName,frequency,week,quarter,
	...
	@RequestMapping(value="/privateSectorupdate", method=RequestMethod.POST)
	public @ResponseBody void doUpdate(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException, JSONException, SHDRCException{
		try{
			log.debug(this.getClass().getName() + "- Entering ");
			PrintWriter out = response.getWriter();
			String handsOnTableData=request.getParameter("handsOnTableData");
			JSONArray jsonArray = new JSONArray(handsOnTableData);
			boolean successFlag = privateSectorDataEntryManager.updatePvtSecDirectorateRecords(jsonArray);
			if(successFlag)
