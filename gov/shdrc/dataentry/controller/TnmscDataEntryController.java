...
@Controller
public class TnmscDataEntryController {
	...
	@RequestMapping(value="/tnmscDataEntry", method=RequestMethod.GET)
	public ModelAndView onLoad(HttpServletRequest request) throws SHDRCException, NoSuchAlgorithmException, ServletException{		
		...
		try { 
		    ...
		    UserInfo.hasValidDataEntryAccess(ShdrcConstants.Role.TNMSC);
		    modelView.setViewName("tnmscDataentry");			
	        ...
	        institutionList=  tnmscDataEntryManager.getInstitutionList();
	       ...
	       modelView.addObject("institutionName", institutionName);
	        ...
	        List<CommonStringList> classificationList= commonManager.getClassificationList(ShdrcConstants.DirectorateId.TNMSC,ShdrcQueryList.TNMSC_CLASSIFICATION_LIST);
	        ...
	        modelView.addObject("directorateId", ShdrcConstants.DirectorateId.TNMSC);     
	...
	@RequestMapping(value="/tnmscsearch", method=RequestMethod.POST)
	public @ResponseBody void search(HttpServletRequest request,HttpServletResponse response) throws SHDRCException, IOException, JSONException{
		try{
			...
			JSONArray tnmscDirectorateRecords =null;
			
				 classificationList=commonManager.getGeneralCategoryListByFrequency(ShdrcConstants.DirectorateId.TNMSC,frequency,dataEntryLevel,ShdrcQueryList.TNMSC_GENERAL_CATEGORY_LIST_BY_FREQUENCY);
				 if(classificationList!=null){
			        	CommonStringList commonStringList = classificationList.get(0);
			        	classificationName = commonStringList.getName();
			        	tnmscDirectorateRecords = tnmscDataEntryManager.getTnmscDirectorateRecords(districtId,institutionId,classificationName,frequency,week,quarter,
				   				 searchDate, month, year);
			       
			        	if(tnmscDirectorateRecords==null){
			    			tnmscDirectorateRecords = tnmscDataEntryManager.getIndicatorList(ShdrcConstants.DirectorateId.TNMSC,classificationName,frequency,"N");
			    		}			
			}
			...
			obj.put("tnmscDirectorateRecords", tnmscDirectorateRecords);
	...
	@RequestMapping(value="/tnmsctabChange", method=RequestMethod.POST)
	public @ResponseBody void onTabChange(HttpServletRequest request,HttpServletResponse response) throws SHDRCException, IOException, JSONException{
		try{
			...
			obj.put("isFrequencySelectionValid", true);
			json=new Gson().toJson(classificationList);
			JSONArray tnmscDirectorateRecords = tnmscDataEntryManager.getTnmscDirectorateRecords(districtId,institutionId,classificationName,frequency,week,quarter,
					 searchDate, month, year);
		
			if(tnmscDirectorateRecords==null && classificationName!=null){
				tnmscDirectorateRecords = tnmscDataEntryManager.getIndicatorList(ShdrcConstants.DirectorateId.TNMSC,classificationName,frequency,"N");
			}
			obj.put("tnmscDirectorateRecords", tnmscDirectorateRecords);
			obj.put("genearlCategoryList", json);
	...
	@RequestMapping(value="/tnmscsave", method=RequestMethod.POST)
	public @ResponseBody void doSave(HttpServletRequest request,HttpServletResponse response) throws SHDRCException, IOException, ServletException, JSONException{	
		...
		try{
			...
			boolean successFlag =tnmscDataEntryManager.insertTnmscDirectorateRecords(districtId,institutionId, classificationName,frequency,week,quarter,
	...
	@RequestMapping(value="/tnmscupdate", method=RequestMethod.POST)
	public @ResponseBody void doUpdate(HttpServletRequest request,HttpServletResponse response) throws SHDRCException, IOException, ServletException, JSONException{
		try{
			...
			boolean successFlag = tnmscDataEntryManager.updateTnmscDirectorateRecords(jsonArray);
			if(successFlag)
