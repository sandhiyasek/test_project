...
@Controller
public class MrbDataEntryController {
	private static Logger log= Logger.getLogger(MrbDataEntryController.class);
	@Autowired
	IMrbDataEntryManager mrbDataEntryManager;
	@Autowired
	ICommonManager commonManager;
	
	@RequestMapping(value="/mrbDataEntry", method=RequestMethod.GET)
	public ModelAndView onLoad(HttpServletRequest request) throws NoSuchAlgorithmException, ServletException, SHDRCException{		
		...
		try { 
		    ...
		    UserInfo.hasValidDataEntryAccess(ShdrcConstants.Role.MRB);
		    modelView.setViewName("mrbDataentry");			
	        ...
	        List<CommonStringList> classificationList= commonManager.getClassificationList(ShdrcConstants.DirectorateId.MRB,ShdrcQueryList.MRB_CLASSIFICATION_LIST);
	        ...
	        modelView.addObject("directorateId", ShdrcConstants.DirectorateId.MRB);
	...
	@RequestMapping(value="/mrbsearch", method=RequestMethod.POST)
	public @ResponseBody void search(HttpServletRequest request,HttpServletResponse response) throws SHDRCException, IOException, JSONException{
		try { 
			...
			JSONArray mrbDirectorateRecords =null;
		
			 classificationList=commonManager.getGeneralCategoryListByFrequency(ShdrcConstants.DirectorateId.MRB,frequency,dataEntryLevel,ShdrcQueryList.MRB_GENERAL_CATEGORY_LIST_BY_FREQUENCY);
			 if(classificationList!=null){
		        	CommonStringList commonStringList = classificationList.get(0);
		        	classificationName = commonStringList.getName();
		        	mrbDirectorateRecords = mrbDataEntryManager.getMrbDirectorateRecords(districtId,institutionId,classificationName,frequency,week,quarter,
		   				 searchDate, month, year);
		        	if(mrbDirectorateRecords==null){
		    			mrbDirectorateRecords = mrbDataEntryManager.getIndicatorList(ShdrcConstants.DirectorateId.MRB,classificationName,frequency,"N",null);
		    		}
			 }		
			...
			obj.put("mrbDirectorateRecords", mrbDirectorateRecords);
	...
	@RequestMapping(value="/mrbtabChange", method=RequestMethod.POST)
	public @ResponseBody void onTabChange(HttpServletRequest request,HttpServletResponse response) throws SHDRCException, IOException, JSONException{
		...
		obj.put("isFrequencySelectionValid", true);
		json=new Gson().toJson(classificationList);
		JSONArray mrbDirectorateRecords = mrbDataEntryManager.getMrbDirectorateRecords(districtId,institutionId,classificationName,frequency,week,quarter,
				 searchDate, month, year);
		
		if(mrbDirectorateRecords==null && classificationName!=null){
			mrbDirectorateRecords = mrbDataEntryManager.getIndicatorList(ShdrcConstants.DirectorateId.MRB,classificationName,frequency,"N",null);
		}
		obj.put("mrbDirectorateRecords", mrbDirectorateRecords);
		obj.put("genearlCategoryList", json);
	...
	@RequestMapping(value="/mrbsave", method=RequestMethod.POST)
	public @ResponseBody void doSave(HttpServletRequest request,HttpServletResponse response) throws SHDRCException, IOException, ServletException, JSONException{
		...
		PrintWriter out = response.getWriter();
			...
			boolean successFlag =mrbDataEntryManager.insertMrbDirectorateRecords(districtId,institutionId, classificationName,frequency,week,quarter,
	...
	@RequestMapping(value="/mrbupdate", method=RequestMethod.POST)
	public @ResponseBody void doUpdate(HttpServletRequest request,HttpServletResponse response) throws SHDRCException, IOException, ServletException, JSONException{
		try{
			...
			boolean successFlag = mrbDataEntryManager.updateMrbDirectorateRecords(jsonArray);
			if(successFlag)
