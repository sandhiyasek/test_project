...
@Controller
public class CmchisDataEntryController {
	private static Logger log=Logger.getLogger(CmchisDataEntryController.class);
	@Autowired
	ICmchisDataEntryManager cmchisDataEntryManager;
	@Autowired
	ICommonManager commonManager;
	
	@RequestMapping(value="/cmchisDataEntry", method=RequestMethod.GET)
	public ModelAndView onLoad(HttpServletRequest request) throws SHDRCException, ServletException, NoSuchAlgorithmException{		
		...
		try { 
		    ...
		    UserInfo.hasValidDataEntryAccess(ShdrcConstants.Role.CMCHIS);
		    modelView.setViewName("cmchisDataentry");			
	        ...
	        if(Util.isNotNull(districtList)){
	        	...
	        	if(ShdrcConstants.SELECTALLID==district.getId()){
	        		...
	        		institutionList= cmchisDataEntryManager.getInstitutionList(district.getId(),userName);
	        	}	        	
	        ...
	        modelView.addObject("directorateId", ShdrcConstants.DirectorateId.CMCHIS);
	        
	        List<CommonStringList> classificationList= commonManager.getClassificationList(ShdrcConstants.DirectorateId.CMCHIS,ShdrcQueryList.CMCHIS_CLASSIFICATION_LIST);
	...
	@RequestMapping(value="/cmchisdistrictChange", method=RequestMethod.POST)
	public @ResponseBody void getInstitution(HttpServletRequest request,HttpServletResponse response) throws IOException, JSONException,SHDRCException{
		...
		if(Util.isNotNull(request.getParameter("districtId"))){
			...
			institutionList= cmchisDataEntryManager.getInstitutionList(districtId,userName);
		}else{
			institutionList = new ArrayList<CommonStringList>();
			CommonStringList commonStringList = new CommonStringList();
			commonStringList.setId(ShdrcConstants.SELECTALLID);
	...
	@RequestMapping(value="/cmchissearch", method=RequestMethod.POST)
	public @ResponseBody void search(HttpServletRequest request,HttpServletResponse response) throws IOException, JSONException,SHDRCException{
		...
		JSONArray cmchisDirectorateRecords =null;
	
		 classificationList=commonManager.getGeneralCategoryListByFrequency(ShdrcConstants.DirectorateId.CMCHIS,frequency,dataEntryLevel,ShdrcQueryList.CMCHIS_GENERAL_CATEGORY_LIST_BY_FREQUENCY);
		 if(classificationList!=null){
	        	CommonStringList commonStringList = classificationList.get(0);
	        	classificationName = commonStringList.getName();
	        	cmchisDirectorateRecords = cmchisDataEntryManager.getCmchisDirectorateRecords(districtId,institutionId,classificationName,frequency,week,quarter,
	   				 searchDate, month, year);
	        	if(cmchisDirectorateRecords==null){
	    			cmchisDirectorateRecords = cmchisDataEntryManager.getIndicatorList(ShdrcConstants.DirectorateId.CMCHIS,classificationName,frequency,"N",null);
	    		}
		 } 			
		...
		obj.put("cmchisDirectorateRecords", cmchisDirectorateRecords);
	...
	@RequestMapping(value="/cmchistabChange", method=RequestMethod.POST)
	public @ResponseBody void onTabChange(HttpServletRequest request,HttpServletResponse response) throws IOException, JSONException,SHDRCException{
		...
		obj.put("isFrequencySelectionValid", true);
		json=new Gson().toJson(classificationList);
		JSONArray cmchisDirectorateRecords = cmchisDataEntryManager.getCmchisDirectorateRecords(districtId,institutionId,classificationName,frequency,week,quarter,
				 searchDate, month, year);
		if(cmchisDirectorateRecords==null && classificationName!=null){
			cmchisDirectorateRecords = cmchisDataEntryManager.getIndicatorList(ShdrcConstants.DirectorateId.CMCHIS,classificationName,frequency,"N",null);
		}
		obj.put("cmchisDirectorateRecords", cmchisDirectorateRecords);
		obj.put("genearlCategoryList", json);
	...
	@RequestMapping(value="/cmchissave", method=RequestMethod.POST)
	public @ResponseBody void doSave(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException,SHDRCException, JSONException{
		...
		PrintWriter out = response.getWriter();
			...
			boolean successFlag =cmchisDataEntryManager.insertCmchisDirectorateRecords(districtId,institutionId, classificationName,frequency,week,quarter,
	...
	@RequestMapping(value="/cmchisupdate", method=RequestMethod.POST)
	public @ResponseBody void doUpdate(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException,SHDRCException, JSONException{

			log.debug(this.getClass().getName() + "- Entering ");
			PrintWriter out = response.getWriter();
			String handsOnTableData=request.getParameter("handsOnTableData");
			JSONArray jsonArray = new JSONArray(handsOnTableData);
			boolean successFlag = cmchisDataEntryManager.updateCmchisDirectorateRecords(jsonArray);
			if(successFlag)
