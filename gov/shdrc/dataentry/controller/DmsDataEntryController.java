...
@Controller
public class DmsDataEntryController {
	private static Logger log=Logger.getLogger(DmsDataEntryController.class);
	@Autowired
	IDmsDataEntryManager dmsDataEntryManager;
	@Autowired
	ICommonManager commonManager;
	
	@RequestMapping(value="/dmsDataEntry", method=RequestMethod.GET)
	public ModelAndView onLoad(HttpServletRequest request) throws SHDRCException, NoSuchAlgorithmException, ServletException, UnsupportedEncodingException{		
		...
		try { 
		    ...
		    modelView.setViewName("dmsDataentry");			
		    UserInfo.hasValidDataEntryAccess(ShdrcConstants.Role.DMS);
	        ...
	        if(Util.isNotNull(districtList)){
	        	...
	        	if(ShdrcConstants.SELECTALLID==district.getId()){
	        		...
	        		hospitalList=  dmsDataEntryManager.getHospitalList(district.getId(),userName);
	        	}
	        	
	        	  modelView.addObject("hospitalList", hospitalList);
	        }
			
	        if(hospitalList!=null && hospitalList.size()>0){
	        	CommonStringList institution = hospitalList.get(0);
			...
			List<CommonStringList> classificationList= commonManager.getClassificationList(ShdrcConstants.DirectorateId.DMS,ShdrcQueryList.DMS_CLASSIFICATION_LIST);
	        ...
	        modelView.addObject("directorateId", ShdrcConstants.DirectorateId.DMS);   
	...
	@RequestMapping(value="/dmsdistrictChange", method=RequestMethod.POST)
	public @ResponseBody void getHospital(HttpServletRequest request,HttpServletResponse response) throws IOException, JSONException,SHDRCException{
		...
		try{
			...
			if(Util.isNotNull(request.getParameter("districtId"))){
				...
				hospitalList= dmsDataEntryManager.getHospitalList(districtId,userName);
			}else{
				hospitalList = new ArrayList<CommonStringList>();
				CommonStringList commonStringList = new CommonStringList();
				commonStringList.setId(ShdrcConstants.SELECTALLID);
	...
	@RequestMapping(value="/dmssearch", method=RequestMethod.POST)
	public @ResponseBody void search(HttpServletRequest request,HttpServletResponse response) throws IOException, JSONException,SHDRCException{
		try{
			log.debug(this.getClass().getName() + "- Entering ");
			Integer districtId=(Util.isNotNull(request.getParameter("districtId"))?Integer.parseInt(request.getParameter("districtId")):null);
			Integer hospitalId=(Util.isNotNull(request.getParameter("hospitalId"))?Integer.parseInt(request.getParameter("hospitalId")):null);
			...
			String dataEntryLevel = Util.getDataEntryLevel(districtId,null,hospitalId);
			String classificationName =null;
			List<CommonStringList> classificationList=null;
			JSONArray dmsDirectorateRecords =null;
			
			classificationList=commonManager.getGeneralCategoryListByFrequency(ShdrcConstants.DirectorateId.DMS,frequency,dataEntryLevel,
					ShdrcQueryList.DMS_GENERAL_CATEGORY_LIST_BY_FREQUENCY);
			if(classificationList!=null){
				CommonStringList commonStringList = classificationList.get(0);
				classificationName = commonStringList.getName();
				dmsDirectorateRecords = dmsDataEntryManager.getDmsDirectorateRecords(districtId,hospitalId,classificationName,frequency,week,quarter,searchDate, month, year);
				if(dmsDirectorateRecords==null){
					dmsDirectorateRecords = dmsDataEntryManager.getIndicatorList(ShdrcConstants.DirectorateId.DMS,classificationName,frequency,"N");
				}
			}		
	...
	@RequestMapping(value="/dmstabChange", method=RequestMethod.POST)
	public @ResponseBody void onTabChange(HttpServletRequest request,HttpServletResponse response) throws IOException, JSONException,SHDRCException{
		try{
			log.debug(this.getClass().getName() + "- Entering ");
			...
			obj.put("isFrequencySelectionValid", true);
			json=new Gson().toJson(classificationList);
			JSONArray dmsDirectorateRecords = dmsDataEntryManager.getDmsDirectorateRecords(districtId,hospitalId,classificationName,frequency,week,quarter,
					 searchDate, month, year);
			
			if(dmsDirectorateRecords==null && classificationName!=null){
				dmsDirectorateRecords = dmsDataEntryManager.getIndicatorList(ShdrcConstants.DirectorateId.DMS,classificationName,frequency,"N");
			}
			obj.put("dmsDirectorateRecords", dmsDirectorateRecords);
			obj.put("genearlCategoryList", json);
	...
	@RequestMapping(value="/dmssave", method=RequestMethod.POST)
	public @ResponseBody void doSave(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException,SHDRCException, JSONException{
		...
		PrintWriter out = response.getWriter();
			...
			boolean successFlag =dmsDataEntryManager.insertDmsDirectorateRecords(districtId,hospitalId, classificationName,frequency,week,quarter,
	...
	@RequestMapping(value="/dmsupdate", method=RequestMethod.POST)
	public @ResponseBody void doUpdate(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException, JSONException{
		try{
			log.debug(this.getClass().getName() + "- Entering ");
			PrintWriter out = response.getWriter();
			String handsOnTableData=request.getParameter("handsOnTableData");
			JSONArray jsonArray = new JSONArray(handsOnTableData);
			boolean successFlag = dmsDataEntryManager.updateDmsDirectorateRecords(jsonArray);
			if(successFlag)
